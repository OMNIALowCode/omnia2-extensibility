...
Function: The standard file used to obtain queries to the Primavera ERP. This script receives a SQL query string from the system, as well as all the information necessary to authenticate itself with a specific company in the ERP Platform, and returns the result of the SQL query.
...
namespace myMIS
{
    public class Script
    {
        ...
        private ScriptResponse executeQuery(ContextData context, Entity document, string query)
        {
            ...
            try
            {
                bsPlat = OpenPlatform(externalSystem.Code, (string)externalSystem.Parameters["Username"], (string)externalSystem.Parameters["Password"], tipoPlataforma);
            }
            catch (Exception e)
            {
            ...
            StdBELista queryResults = bsPlat.Registos.Consulta(query);
        ...
        private StdPlatBS OpenPlatform(string company, string username, string password, EnumTipoPlataforma tp)
        {
            StdPlatBS bsPlat = new StdPlatBS();
            StdBSConfApl platConfig = new StdBSConfApl();
            platConfig.AbvtApl = "ERP";
            platConfig.Instancia = "default";
            platConfig.Utilizador = username;
            platConfig.PwdUtilizador = password;
            platConfig.LicVersaoMinima = "09.00";

            StdBETransaccao trans = null;
            bsPlat.AbrePlataformaEmpresa(company, trans, platConfig, tp, string.Empty);
            return bsPlat;
        }
    }
}
